export default function ReportTable({ reports, hours }) {
    ...
    function CreateReportTable() {
        ...
        for (let i = 0; i < reports.length; i++) {
            locationsales.push(reports[i].hourly_sales)
        ...
        return (
            <>
                <table className="flex-center ...  w-full py-10 my-12 border-r  md:px-32 shadow-2xl bg-green-600">
                    ...
                    <tbody>
                        {reports.map(value =>
                            <tr key={value.id} className="odd:bg-green-400 even:bg-green-200" >
                                ...
                                <td ><button className="justify-center w-8 h-10" onClick={() => { reports.splice(parseInt(value.id))}}>  <svg xmlns="http://www.w3.org/2000/svg" className="w-8 h-8 "  fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <tfoot className="bg-green-600 border-t border-b border-gray-700">
                        <tr>
                            ...
                            <th className="border-r border-gray-600 "> {console.log(totalsArray)}{(totalsArray.reduce(function (a, b) { return a + b; }, 0)).toFixed(3)} </th>
                        </tr>
                    </tfoot>
                </table>
            </>
